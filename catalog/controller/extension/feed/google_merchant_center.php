...
class ControllerExtensionFeedGoogleMerchantCenter extends Controller
{
    public function index()
    {
        ...
        if ($this->get($prefix.'status')) {
            ...
            //shipping carrier & store country
            $shipping_price = null;
            $shipping_carrier = array();
            $shipping_country = array();
            if (isset($_GET['shipping_price'])) {//url parameter override
                $shipping_price = (float)$_GET['shipping_price'];
                $file_name_append.="_sp".$_GET['shipping_price'];
            } elseif ((int)$this->get('shipping_'.$this->get($prefix.'carrier').'_status')) {
                $shipping_carrier['code'] = $this->get($prefix.'carrier');
                $country_id = (int)$this->get('config_country_id');
                $zone_id = (int)$this->get('config_zone_id');
                $shipping_country[$country_id]['country']=$this->model_localisation_country->getCountry($country_id);
                ...
                $shipping_country[$country_id]['zones'][$zone_id]=$zone_info;
                $shipping_country[$country_id]['zones'][$zone_id]['iso_code_2'] = $shipping_country[$country_id]['country']['iso_code_2'];
                $shipping_country[$country_id]['zones'][$zone_id]['iso_code_3'] = $shipping_country[$country_id]['country']['iso_code_3'];
            }
            ...
            if (!$save_to_file || ($save_to_file && ($start === 0 || $start === $range_start))) {
                ...
                if ($isDefaultLang) {
                    $meta_description = $this->get('config_meta_description');
            ...
            while (count($products)>0) {
                foreach ($products as $product) {
                    ...
                    if (isset($category_id_google['taxonomy_id'])) {
                        $google_product_category = $category_id_google['taxonomy_id'];
                        $is_apparel = $this->model_extension_feed_google_merchant_center->isApparel($google_product_category);
                    } else {
                        $google_product_category = reset($base_taxonomy);
                        if ((int)$google_product_category) {
                            $is_apparel = $this->model_extension_feed_google_merchant_center->isApparel($google_product_category);
                        }
                    }
                    ...
                    foreach ($options as $key => $option_group) {
                        ...
                        if (!empty($shipping_carrier)) {
                            $this->cart->setCustomProduct(
                                array(
                                    array_merge(
                                        array('cart_id' => 0),
                                        array('option' => array($option)),
                                        array('total' => $product['minimum']*(float)($sale_price > 0 ? $sale_price : $price)),
                                        array_merge(
                                            $product,
                                            array('quantity' => $product['minimum'], 'option' => array(array('quantity' => $product['minimum'])))
                                        )
                                    )
                                )
                            );
                            $shipping_data = $this->model_extension_feed_google_merchant_center->getShippingInfo($shipping_country, array($shipping_carrier), 1);
                            $this->cart->setCustomProduct(null);
                            while (!empty($shipping_data) && $shipping_data !== false) {
                                $shipping_data = reset($shipping_data);
                                if (array_key_exists('cost', $shipping_data)) {
                                    $shipping_price = $this->tax->calculate($shipping_data['cost'], $shipping_data['tax_class_id'], $use_tax);
                                    break;
                                }
                            }
                        }
                        ...
                        if ($shipping_price !== null) {//shipping flat rate, use merchant account instead
                            ...
                            $shipping ="<g:shipping><g:price>".$shipping_price.' '.$currency_code."</g:price></g:shipping>";
                        }
...
<item>';
                        ...
                        $output .= '<g:image_link>'.($option_image_link == null ? $image_link : $option_image_link).'</g:image_link>';
                        $output .= '<g:additional_image_link>'.implode('</g:additional_image_link><g:additional_image_link>',$additional_image_link).'</g:additional_image_link>';
                        $output .= '<g:price>'.$price.'</g:price>';
                        $output .= '<g:sale_price>'.$sale_price.'</g:sale_price>';
                        $output .= '<g:sale_price_effective_date>'.$sale_price_effective_date.'</g:sale_price_effective_date>';
                        $output .= '<g:product_type><![CDATA['.implode(',',$product_type).']]></g:product_type>';
                        $output .= '<g:google_product_category><![CDATA['.$google_product_category.']]></g:google_product_category>';
                        $output .= '<g:availability>'.$availability.'</g:availability>';
                        $output .= $shipping;
                        $output .= $tax_rate;
                        $output .= '<g:shipping_weight>'.$shipping_weight.'</g:shipping_weight>';
                        $output .= '<g:condition>'.$condition.'</g:condition>';
                        $output .= '<g:size><![CDATA['.implode(',', $size).']]></g:size>';
                        $output .= '<g:color><![CDATA['.implode(',', $color).']]></g:color>';
                        $output .= '<g:pattern><![CDATA['.implode(',', $pattern).']]></g:pattern>';
                        $output .= '<g:material><![CDATA['.implode(',', $material).']]></g:material>';
                        $output .= '<g:age_group>'.$age_group.'</g:age_group>';
                        $output .= '<g:gender>'.$gender.'</g:gender>';
                        $output .= '</item>';
                    }
                }
            ...
            if ($save_to_file) {
                ...
                header('Content-Disposition: inline; filename="google_merchant_center'.$file_name_append.'.xml"');
                print($output);
                exit(0);
            }
        } else {
